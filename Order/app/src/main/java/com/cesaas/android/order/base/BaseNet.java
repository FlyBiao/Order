...
public class BaseNet {
	...
	public void mPostNet() {
		if (App.mInstance().getNetType() != NetworkUtil.NO_NETWORK) {
			...
			Log.i(Constant.TAG,"参数:"+ Constant.IP + uri + "\n" + data+ "\n");

			HttpUtils http = new HttpUtils();
			http.send(HttpMethod.POST, Constant.IP + uri, params, new RequestCallBack<HttpUtils>() {
				@Override
			...
			ToastUtils.show(App.mInstance(), "未Intent网络，请检查后重试！", ToastUtils.CENTER);
		}
	}
	...
	protected void mFail(HttpException e, String err) {
		...
		Log.i(Constant.TAG,"\n\n" +e+"err："+err);
		ToastUtils.show(mContext, "服务器连接或返回错误！", ToastUtils.CENTER);
	}
